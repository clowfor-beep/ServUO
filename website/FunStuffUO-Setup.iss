...
begin
  UOPathPage := CreateInputDirPage(
    ...
    'Select the folder where you installed and patched the UO client (uo.exe).',
    False, '');
  UOPathPage.Add('');
...
begin
  ...
  RunPS(
    ...
    'Expand-Archive -Path "$env:TEMP\cuo.zip" -DestinationPath "' + InstallDir + '" -Force;' +
  ...
  RunPS(
    ...
    '$exe | Set-Content -Path "' + TmpPath + '\razorpath.txt" -Encoding UTF8'
  );

  // Read plugin path from temp file
  ...
  // ── Step 3: Set up launcher database ─────────────────────────────────────
  SetStatus('Configuring launcher profile...');
  ...
  // Clear old profiles and insert fresh one
  ...
  // Get profile ID and insert plugin using PowerShell (avoids arg length limits)
  RunPS(
    '$db = "' + LauncherDB + '";' +
    '$sq = "' + SqlitePath + '";' +
