...
namespace Server.Engines.Quests
{
    public class WondersOfTheNaturalWorldQuest : BaseQuest
    {
            ...
            : base()
        {
            ...
            // No AddReward — we hand the ticket directly in GiveRewards() so no gump is needed
        }
        ...
        // Complete = null so QuestHelper bypasses the reward gump and calls GiveRewards() directly
        public override object Complete => null;

        public override void GiveRewards()
        {
            var ticket = new PrismOfLightAdmissionTicket();

            if (!Owner.PlaceInBackpack(ticket))
                ticket.MoveToWorld(Owner.Location, Owner.Map);

            Owner.SendLocalizedMessage(1074360, "#1074340"); // You receive a reward: Prism of Light Admission Ticket
            Owner.SendMessage(0x35, "Lefty hands you an admission ticket. Enjoy the caverns!");
