# ============================================================
# LethalDartsQuest_v18.py — Razor Enhanced Python
...
def nav_gump(pause_ms=500):
    # For SendAction navigation — server responds so fast WaitForGump misses it.
    # Pause first so the gump is already there, then check HasGump.
    Misc.Pause(pause_ms)
    if Gumps.HasGump():
        return Gumps.CurrentGump()
...
def get_lethal_darts_quest(vendor_serial):
    for attempt in range(100):
        ...
        # Tier 2: navigate to Objectives where item name is plain AddLabel text
        Gumps.ResetGump()
        ...
        # Wrong quest — refuse and dismiss the Refuse dialogue cleanly
